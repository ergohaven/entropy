...
impl EntropyApp {
    ...
    pub(super) fn draw_layout_settings_dropdown(
        ...
        if let Some(settings_rect) = settings_tab_rect {
            ...
            let deferred_modules = false;
            #[cfg(not(target_arch = "wasm32"))]
            let deferred_touchpad = self.deferred_section_available(DeferredLoadSection::Touchpad);
            #[cfg(target_arch = "wasm32")]
            let deferred_touchpad = false;
            #[cfg(not(target_arch = "wasm32"))]
            let deferred_bluetooth =
                self.deferred_section_available(DeferredLoadSection::Bluetooth);
            #[cfg(target_arch = "wasm32")]
            let deferred_bluetooth = false;
            #[cfg(not(target_arch = "wasm32"))]
            let deferred_layer_leds =
                self.deferred_section_available(DeferredLoadSection::LayerLeds);
            #[cfg(target_arch = "wasm32")]
            let deferred_layer_leds = false;
            #[cfg(not(target_arch = "wasm32"))]
            let deferred_rgb = self.deferred_section_available(DeferredLoadSection::Rgb);
            #[cfg(target_arch = "wasm32")]
